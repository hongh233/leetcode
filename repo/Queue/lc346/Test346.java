...
public class Test346 {
    ...
    public static String runTest(String input) {
        ...
        Solution346.MovingAverage ma = null;

        for (int i = 0; i < ops.length; i++) {
            switch (ops[i]) {
                case "MovingAverage" -> {
                    int size = Integer.parseInt(args[i][0]);
                    ma = new Solution346.MovingAverage(size);
                    out.add("null");
                }
                case "next" -> {
                    int v = Integer.parseInt(args[i][0]);
                    double ans = ma.next(v);
                    out.add(formatDouble(ans));
                }
    ...
    private static String formatDouble(double x) {
        BigDecimal bd = BigDecimal.valueOf(x).setScale(5, BigDecimal.ROUND_HALF_UP);
