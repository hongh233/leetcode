...
public class Test146 {
    public static String runTest(String input) {
        ...
        Solution146.LRUCache cache = null;

        for (int i = 0; i < ops.length; i++) {
            String op = ops[i];
            switch (op) {
                case "LRUCache" -> {
                    int capacity = Integer.parseInt(args[i][0]);
                    cache = new Solution146.LRUCache(capacity);
                    out.add("null");
                }
                case "put" -> {
                    int key = Integer.parseInt(args[i][0]);
                ...
                case "get" -> {
                    int key = Integer.parseInt(args[i][0]);
                    out.add(String.valueOf(cache.get(key)));
                }
