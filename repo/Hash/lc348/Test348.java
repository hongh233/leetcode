...
public class Test348 {
    public static String runTest(String input) {
        ...
        Solution348.TicTacToe game = null;

        for (int i = 0; i < ops.length; i++) {
            switch (ops[i]) {
                case "TicTacToe" -> {
                    int n = Integer.parseInt(args[i][0]);
                    game = new Solution348.TicTacToe(n);
                    out.add("null");
                }
                case "move" -> {
                    int row = Integer.parseInt(args[i][0]);
                    int col = Integer.parseInt(args[i][1]);
                    int player = Integer.parseInt(args[i][2]);
                    out.add(String.valueOf(game.move(row, col, player)));
                }
