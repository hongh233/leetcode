...
public class Test380 {
    public static String runTest(String input) {
        ...
        Solution380.RandomizedSet rs = null;

        for (int i = 0; i < ops.length; i++) {
            switch (ops[i]) {
                case "RandomizedSet" -> {
                    rs = new Solution380.RandomizedSet();
                    out.add("null");
                }
                case "insert" -> {
                    int v = Integer.parseInt(args[i][0]);
                    out.add(String.valueOf(rs.insert(v)));
                }
                case "remove" -> {
                    int v = Integer.parseInt(args[i][0]);
                    out.add(String.valueOf(rs.remove(v)));
                }
                case "getRandom" -> {
                    out.add(String.valueOf(rs.getRandom()));
                }
